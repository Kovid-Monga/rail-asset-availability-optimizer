...
export const EngineeringRequestList = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-rail-border">
        <div>
          <h2 className="text-lg font-bold text-rail-text tracking-tight flex items-center gap-2">
            <span>Engineering Maintenance Requirements & Block Registry</span>
      ...
      <Card className="p-4 bg-rail-surface">
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 w-full">
            ...
            <input
              type="text"
              placeholder="Search by Request ID (e.g. ENG-2026-00421), asset name, location..."
              value={searchQuery}
      ...
      {activeRequestForModal && (
            ...
            <div className="flex items-center justify-between w-full">
              ...
              <div className="flex items-center gap-2">
                ...
                {activeRequestForModal.status !== REQUEST_STAGES.HUMAN_REVIEW_REQUESTED && (
                  <Link to={`/eng/human-review?id=${activeRequestForModal.id}`}>
        ...
        >
          <div className="space-y-6">
            {/* Status Timeline strictly adhering to exact PRD sequence */}
            ...
            {/* AI Decision & Plain-Language Explanation */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              <div className="p-2.5 bg-[#FAF7F0] border border-rail-border rounded-sm">
                <span className="text-[10px] uppercase text-rail-muted block">Corridor Section</span>
                <span className="font-semibold text-rail-text mt-0.5 block">{activeRequestForModal.corridor}</span>
              </div>
              <div className="p-2.5 bg-[#FAF7F0] border border-rail-border rounded-sm">
                <span className="text-[10px] uppercase text-rail-muted block">Track Line</span>
                <span className="font-semibold text-rail-text mt-0.5 block">{activeRequestForModal.trackLine}</span>
              </div>
              <div className="p-2.5 bg-[#FAF7F0] border border-rail-border rounded-sm">
                <span className="text-[10px] uppercase text-rail-muted block">Speed Restriction</span>
                <span className="font-semibold text-rail-text mt-0.5 block">
                  {activeRequestForModal.speedRestrictionRequired ? activeRequestForModal.speedRestrictionValue : 'None'}
                </span>
              </div>
              <div className="p-2.5 bg-[#FAF7F0] border border-rail-border rounded-sm">
                <span className="text-[10px] uppercase text-rail-muted block">Power Disconnection</span>
                <span className="font-semibold text-rail-text mt-0.5 block">
                  {activeRequestForModal.powerDisconnectionRequired ? '25kV Isolation Required' : 'Not Required'}
                </span>
              </div>
            </div>
          </div>
