...
export const EngineeringDashboard = () => {
  ...
  const scheduledCount = requests.filter(r => r.status === REQUEST_STAGES.SCHEDULED).length;
  ...
  // Overdue flagged from TMS source system
  const overdueCount = requests.filter(r => r.isOverdue).length + tmsAssets.filter(a => a.isOverdue).length;
    ...
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="font-bold text-base text-rail-text">
              Engineering Division • Track Maintenance & Civil Operations
            </span>
            <span className="text-[10px] font-mono px-2 py-0.5 bg-[#E0D7C6] text-rail-text rounded-sm border border-rail-borderDark font-semibold">
              TMS Connected
            </span>
          </div>
          <p className="text-xs text-rail-muted mt-1 max-w-3xl">
            <strong className="text-rail-text font-semibold">Core Principle:</strong> Engineering declares physical site requirements, track possession windows, and equipment constraints. The AI Prioritization Layer and Optimization Engine automatically determine optimal block allocation and cross-department bundling.
          </p>
        </div>
        <div className="flex items-center gap-2.5 shrink-0">
          <Link to="/eng/new-request">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2.5">
        ...
        <div className="bg-rail-surface border border-rail-border p-3 rounded-md">
          ...
          <span className="text-[10px] text-rail-muted mt-0.5 block">TMS Ingested</span>
        </div>

        {/* Under ML Prioritization */}
        <div className="bg-rail-surface border border-rail-border p-3 rounded-md">
        ...
        {/* Could Not Be Accommodated / Alternatives */}
        <div className="bg-rail-surface border border-rail-border p-3 rounded-md">
        ...
        <div className="bg-rail-criticalLight/50 border border-rail-critical/40 p-3 rounded-md">
          ...
          <span className="text-[10px] text-rail-critical font-medium mt-0.5 block">TMS cycle exceeded</span>
        </div>
      </div>

      {/* Corridor Spatial Visualization */}
      <CorridorVisualization activeBlocks={requests} />

      {/* Two Column Layout: Urgent Requests & Notifications */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left 2 Cols: Active & Pending Maintenance Blocks */}
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>
                <span>Active Track Maintenance Requirements</span>
                <span className="text-xs font-normal text-rail-muted">({requests.length} records)</span>
              </CardTitle>
              <Link to="/eng/requests" className="text-xs text-rail-primary hover:underline font-semibold flex items-center gap-1">
                  ...
                  <div key={req.id} className="p-4 hover:bg-[#F9F6EE] transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {req.status === REQUEST_STAGES.ALTERNATIVE_SUGGESTED ? (
                        <Link to="/eng/scheduled-work">
                          <Button size="sm" variant="warning">
                            Review Alternatives
                          </Button>
                        </Link>
                      ) : (
                        <Link to={`/eng/requests?id=${req.id}`}>
          ...
          {/* Overdue Track Defects Flagged by TMS */}
          <Card className="border-rail-warning/50">
            <CardHeader className="bg-[#FAF4E6]">
              <CardTitle className="text-rail-warning flex items-center gap-2">
                ...
                <span>TMS Source System Alerts: Overdue Track Inspections</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 space-y-3">
              {tmsAssets.filter(a => a.isOverdue || a.healthStatus === 'Critical').map(asset => (
                <div key={asset.id} className="p-3 bg-[#FAF7ED] border border-rail-border rounded-sm flex items-start justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={asset.healthStatus === 'Critical' ? 'critical' : 'warning'}>
                        {asset.healthStatus}
                      </Badge>
                    ...
                    <span className="text-[10px] font-mono text-rail-critical font-semibold mt-1 inline-block">
                      Flagged overdue since {asset.overdueDate || 'Recent cycle'} • Cumulative GMT: {asset.cumulativeGmt}
                    </span>
                  </div>
                  <Link to={`/eng/new-request?assetId=${asset.id}`}>
        ...
        {/* Right 1 Col: Operations Notifications Panel */}
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Clock className="w-4 h-4 text-rail-primary" />
                <span>Engineering Operations Feed</span>
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <div className="divide-y divide-rail-border/60 max-h-[500px] overflow-y-auto">
                {notifications.filter(n => n.department === 'ENG' || n.department === 'GLOBAL').map(notif => (
